...
template <typename Type, u32 Width = 0u, u32 Height = 0u> struct Grid2
{

  Grid2(u32 w, u32 h)
    : width_{w}
    , height_{h}
    , data_(size_t{width_} * height_)
  {
  ...
  Grid2(u32 w, u32 h, const Type& value)
    : width_{w}
    , height_{h}
    , data_(size_t{width_} * height_, value)
  {
    DEBUG_ASSERT(width_ != 0u && height_ != 0u, "constructed unusable grid");
  }
  ...
  {
    INVARIANT(data_.size() == size_t{width_} * height_,
              "size of data array must match grid dimensions");
    return data_.size();
  }

  constexpr bool has(u32 x, u32 y) const { return x < width_ && y < height_; }

  constexpr const Type& operator[](u32 x, u32 y) const
  {
  ...
  constexpr const Type* get(u32 x, u32 y) const
  {
  ...
  template <u32 W, u32 H> bool insert(u32 x, u32 y, const Grid2<Type, W, H>& src)
  {
