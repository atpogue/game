...
std::optional<Terrain::Id> terrain::find(std::string_view name) {
    if (auto it = lookup.find(name); it != lookup.end())
        return std::make_optional(it->second);
    return {};
}

std::optional<Terrain::Id> terrain::create(std::string_view name, Sprite sprite, Terrain::Flag flag) {
    if (data.size() > std::numeric_limits<Terrain::Id>::max()) return {};
    if (!graphics::get_texture(sprite.atlas)) return {};
