#include "action/move.hh"
#include "component/pose.hh"
#include "director.hh"
#include "engine/core/random.hh"
#include "engine/event.hh"
#include "engine/core/math.hh"
#include "engine/main.hh"
#include "engine/render/camera.hh"
#include "engine/render/window.hh"
#include "world/terrain.hh"
#include "world/grassland.hh"
#include "registry.hh"
#include <glm/common.hpp>
#include <random>
#include <SDL3/SDL.h>
#include <SDL3/SDL_main.h>
#include <SDL3/SDL_render.h>

namespace { ///////////////////////////////////////////////////////////////////////////////

    // This all assumes that none of these variables need the engine or SDL to be initialized first

    struct {
        Entity entity;
        PlayerDirector director;
    } player;

    auto seed = random_seed();
    SplitMix64 rng(seed);
    std::uniform_int_distribution<u32> dist{0u, chunk_size-1u};

    GameRegistry registry;

    Chunk chunk;
    Camera camera;

} /////////////////////////////////////////////////////////////////////////////////////////

WindowConfig app_window_config() {
    return { .title = "My Game", .width = 800, .height = 600 };
}

static bool load_world(u32 x, u32 y) {
    GrasslandGenerator(seed).generate(x, y, chunk);
    return true;
}

bool load_player() {
    player.entity = registry.create();
    registry.emplace<Pose>(player.entity, Vec2<float>{chunk_size * 0.5f, chunk_size * 0.5f});
    DEBUG_ASSERT(registry.has<Pose>(player.entity));
    return true;
}

void process_input() {
    for (auto command : player.director.generate()) {
        try_submit_command(registry, player.entity, command);
    }
}

bool app_init() {
    camera.viewport = {800.f/tile_size, 600.f/tile_size};
    camera.zoom = 1.3f;
    return load_terrain()
        && load_world(dist(rng), dist(rng))
        && load_player();
}

void app_step() {
    process_input();
    if (auto action = registry.get<MoveAction>(player.entity)) {
        switch (act(registry, player.entity, *action)) {
        case ActionResult::Canceled:
        case ActionResult::Complete:
            registry.erase<MoveAction>(player.entity);
            break;
        case ActionResult::Ongoing:
            break;
        }
    }
}

void app_update(float) {
    auto pose = registry.get<Pose>(player.entity);
    if (pose) camera.position = pose->position;
}

void app_render() {
    chunk.render(camera, tile_size);
}

void app_event(const SDL_Event &event) {
    player.director.event(event);
    switch (event.type) {
    case SDL_EVENT_MOUSE_WHEEL:
        camera.zoom = glm::clamp(camera.zoom + 0.5f * event.wheel.y, 0.7f, 1.9f);
        break;
    case SDL_EVENT_KEY_DOWN:
        switch (event.key.scancode) {
        case SDL_SCANCODE_ESCAPE:
        case SDL_SCANCODE_Q:
            push_event(make_quit_event());
            break;
        case SDL_SCANCODE_SPACE:
            load_world(dist(rng), dist(rng));
            break;
        default:
            break;
        }
        break;
    }
}

void app_quit() {
    registry.clear();
}

