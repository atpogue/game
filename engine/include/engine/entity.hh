...
struct Entity
{
  ...
  constexpr Entity(Handle<> handle) noexcept
    : handle_(handle)
  {
  }
  constexpr Entity& operator=(const Handle<>& handle) noexcept
  {
    handle_ = handle;
    return *this;
  }
  ...
  template <typename Tag = void> [[nodiscard]] constexpr Handle<Tag> to_handle() const noexcept
  {
