...
// An append-only, lookup-by-name set of elements with ID's for quick runtime reference.
template <Named Type>
struct Catalog {
    ...
    void reserve(u32 n) { data_.reserve(n); lookup_.reserve(n); }
...
private:
    ...
    Catalog(const Catalog &) requires std::is_copy_constructible_v<Type> = default;
