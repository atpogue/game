...
namespace lua {
    ...
    // Assumes: [L] is not null, [idx] is non-zero, value at [idx] is a table, [field] is non-zero.
    [[nodiscard]] Result<int> try_push_field(lua_State *L, int type, int idx, unsigned field);

    // Only pushes onto the stack if the expected type is found.
    // Returns: absolute index of the new item, or a content error if the expected type wasn't found.
    // Assumes: [L] is not null, [idx] is non-zero, value at [idx] is a table
    ...
    /* This is supposed to reduce the amount of lua parsing code needed for definitions
    constexpr Field<Rectangle> fields[] = {
        { "x", &Rectangle::x },
        { "y", &Rectangle::y },
        { "w", &Rectangle::w },
        { "h", &Rectangle::h },
    ...
    */
    ...
    template <typename Type>
    using Schema = std::span<const Field<Type>>;

    template <typename Type>
    Result<Type> try_get_fields(lua_State *L, int idx, Schema<Type> schema) {
        Type out;
        for (const auto &field : schema) {
            auto result = std::visit([&](auto member) -> Result<void> {
